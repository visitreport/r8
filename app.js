...
async function insertChunked(table, rows, batchId, chunkSize = 500, concurrency = 4) {
  ...
  async function worker() {
    while (nextIdx < chunks.length && !stopped) {
      const myIdx = nextIdx++;
      const { error } = await sb.from(table).insert(chunks[myIdx]);
      if (error) {
        stopped = true; // หยุดไม่ให้ worker อื่นเริ่ม insert chunk ใหม่เพิ่ม หลังเจอ error แล้ว
        throw error;
      }
    }
  }
...
document.getElementById('btn-process').addEventListener('click', async () => {
  ...
  try {
    ...
    try {
      ...
      // บันทึกไม่สำเร็จกลางทาง — ลบ batch นี้ทิ้ง (cascade ลบแถวที่บันทึกไปบางส่วนด้วย) กันข้อมูลค้างไม่ครบ
      logAppend('บันทึกไม่สำเร็จ กำลังล้างข้อมูลที่บันทึกไปบางส่วน...');
      await sb.from('upload_batches').delete().eq('id', batchId);
      throw insertErr;
    }
